...
async function register(req, res) {
  try {
    ...
    return res.status(201).json(authView.successResponse("Account created successfully", { user: req.session.user }));
  } catch (error) {
...
async function login(req, res) {
  try {
    ...
    if (!isMatch)
      return res.status(401).json(authView.errorResponse("Invalid credentials"));
    req.session.user = { id: user.id, username: user.username, email: user.email };
    return res.status(200).json(authView.successResponse("Login successful", { user: req.session.user }));
  } catch (error) {
    return res.status(500).json(authView.errorResponse("Server error"));
  }
}
...
function guest(req, res) {
  try {
    ...
    return res.status(200).json(authView.successResponse("Guest session created", { guest: true }));
  } catch (error) {
    return res.status(500).json(authView.errorResponse("Server error"));
  }
}
...
function forgotPassword(req, res) {
  try {
    ...
    return res.status(200).json(authView.successResponse("Password reset token created", { token }));
  } catch (error) {
    return res.status(500).json(authView.errorResponse("Server error"));
  }
}
...
function casLogin(req, res) {
  try {
    ...
    return res.status(200).json(authView.successResponse("CAS login successful (mock)", { netid }));
  } catch (error) {
    return res.status(500).json(authView.errorResponse("Server error"));
  }
}
...
function me(req, res) {
  return res.status(200).json(authView.successResponse("Current session retrieved", {
...
module.exports = { register, login, logout, guest, forgotPassword, resetPassword, casLogin, me };
